...
export class ProfilePage {
  constructor(page, userId = 0) {
    ...
    this.avatar = page.locator('img.user-img');
  ...
  async assertUsernameIsVisible(username) {
    await this.step(`User 'username' is shown`, async () => {
      await expect(
  ...
  async assertBioIsVisible(bio) {
    await this.step(`User 'Bio' is shown`, async () => {
      await expect(this.page.locator('p', { hasText: bio })).toBeVisible();
    });
  }
  ...
  async assertAvatarIsSet(url) {
    await this.step(`The 'Avatar' is set`, async () => {
      await expect(this.avatar).toBeVisible();
      await expect(this.avatar).toHaveAttribute('src', url);
    });
  }
}
